# MANGAL Ecosystem — Full Session History (Archive)

> **This is the raw, chronological, un-trimmed session log** — every `§N` entry ever
> written, in original order, nothing removed. It is the source of truth for exact
> commit hashes, byte-level bundle measurements, and step-by-step debugging trails.
>
> **For day-to-day use, read [`/CONTEXT.md`](../CONTEXT.md) instead.** That file is the
> curated, structured summary (vision, architecture, completed-features index, known
> issues, roadmap) distilled from everything below. Come here only when you need the
> full detail behind a specific `§N` reference from `CONTEXT.md`.
